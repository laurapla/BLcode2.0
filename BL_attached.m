...
for i = 2:N
    ...
    X1(i) = X1(i-1)*exp(-b1*beta^2*ds)+A1*(1-exp(-b1*beta^2*ds))*(da+dq*(1/4-e))/(b1*ds);
    X2(i) = X2(i-1)*exp(-b2*beta^2*ds)+A2*(1-exp(-b2*beta^2*ds))*(da+dq*(1/4-e))/(b1*ds);
...
%     % Non-circulatory - AOA contribution
%     X5(i) = X5(i-1)*exp(-dt/(b3*K_alphaM^2*T_I))+A3*da*exp(-dt/(2*b3*K_alphaM^2*T_I));
%     X6(i) = X6(i-1)*exp(-dt/(b4*K_alphaM^2*T_I))+A4*da*exp(-dt/(2*b4*K_alphaM^2*T_I));
...
Cmc_q = -C_Nalpha/(16*beta)*(q-X7)*c/V;
Cmc = Cmc_a+Cmc_q;

% Non-circulatory moment
Cmi_a = -Cni_a/4;
% Cmi_a = -K_alphaM*T_I/M*(b3*X5+b4*X6+dat);
