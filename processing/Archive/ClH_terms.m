...
for j = 1:n_H_array
    ...
    term(:,j,2) = C_Nalpha/2*(alpha-H(j)*k*sin(phi)).*(1+sqrt(x10)).^2/2.*cos(alpha);
    term(:,j,3) = C_Nalpha/2*(alpha-H(j)*k*sin(phi))*2*eta.*(alpha-H(j)*k*sin(phi)).*sqrt(x10).*sin(alpha);
...
Legend = cell(ceil(n_H_array/inter)+1,1);
...
for iter = 1:n_H_array
    ...
    elseif rem(100*H_array(iter),inter)<1e-12
        Legend{ceil(iter/inter)+1} = strcat('$H=$', num2str(H_array(iter)));
    end
end
